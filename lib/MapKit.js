...
(_initialiseProps = function _initialiseProps() {
  ...
  this.initMap = function(props) {
    ...
    if (defaultMapRect) {
      ...
      if (defaultCenter) {
        try {
          mapCenter = _this2.createCoordinate(
            defaultCenter[0],
            defaultCenter[1],
          )
        } catch (e) {
        ...
        if (defaultSpan) {
          try {
            mapSpan = _this2.createCoordinateSpan(
              defaultSpan[0],
              defaultSpan[1],
            )
          } catch (e) {
        ...
        if (mapSpan) {
          ...
          _this2.map.region = _this2.createCoordinateRegion(mapCenter, mapSpan)
        } else {
          // otherwise we just set the center
          _this2.map.center = mapCenter
        }
      }
    }
